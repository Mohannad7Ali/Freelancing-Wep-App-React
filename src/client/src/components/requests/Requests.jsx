...
const Requests = ({ projectId, projectOwnerId, projectData }) => {
  ...
  const mutation = useMutation({
    ...
    onError: (err) => {
      setServerError(err.response?.data?.message || "ربما لديك طلب مرسل بالفعل أو حدث خطأ أثناء إرسال الطلب");
    }
  });
  ...
  const validateForm = () => {
    ...
    // استخدام دالة validateReview للتحقق من نص الطلب
    ...
    if (errors.length > 0) {
      ...
      if (textValidation.ok) {
        setErrors(errors.filter(error => 
          !error.includes("النص") && 
          !error.includes("حرف") && 
          !error.includes("كلمات") &&
          !error.includes("روابط") &&
          !error.includes("بريد") &&
          !error.includes("ألفاظ") &&
          !error.includes("إيموجي") &&
          !error.includes("تكرار")
        ));
      }
    }
  };
